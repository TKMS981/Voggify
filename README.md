...
音楽ファイルを **OGG Vorbis** または **MP3** に変換するデスクトップアプリ。
ドラッグ&ドロップでファイルを積み、形式・品質・出力先を決めて一括変換する。

- 対応入力: MP3 / WAV / FLAC / AAC / M4A / OGG・OGA
...
- **波形をドラッグ**して範囲を選ぶ。両端のハンドルで微調整できる
- 数値欄（`mm:ss.ms`）と波形は双方向に同期する
- 音量は -30dB 〜 +30dB。波形の振幅にも反映される
- 「全体を使う」「0 dB に戻す」で解除
- **▶ 再生**で元のファイルをそのまま再生できる。波形をクリックするとその位置から再生
