...
from .formats import DEFAULT_QUALITY, estimate_output_size, format_estimated_size
...
class FileItem:
    ...
    def estimated_size(self, quality: int = DEFAULT_QUALITY) -> int | None:
        """指定品質での変換後サイズ（バイト）。解析前・エラー時は None。"""
        if self.info is None or self.status is FileStatus.ERROR:
            return None
        return estimate_output_size(
            self.info.duration_sec, quality, self.info.channels
        )

    def display_size(self, quality: int = DEFAULT_QUALITY) -> str:
        """「変換後のサイズ」列の文字列。
        ...
        return format_estimated_size(self.estimated_size(quality))

    def tooltip(self, quality: int = DEFAULT_QUALITY) -> str:
        ...
        if self.info is not None:
            ...
            if self.status is FileStatus.DONE:
                ...
                lines.append(
                    f"変換後の予測: {format_estimated_size(self.estimated_size(quality))}"
                    f" (-q:a {quality})"
                )
        if self.note:
            lines.append("")
